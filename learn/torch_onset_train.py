...
parser.add_argument('--lr', type=float, defualt=1.0, help='Learning rate')
...
def main(_):
    ...
    if args.feat_diff_feet_to_id_fp: # default:None 足で条件付けできるんだろうか
    ...
    if args.feat_diff_coarse_to_id_fp: # default:/data/chart_onset/{pack}/diff_coarse_to_id.txt
        ...
        # 論文中では難易度で条件付けするような内容になっている

    freetext_to_id = None
    if args.feat_freetext_to_id_fp: # default:None(というかこれの使い道なに)
